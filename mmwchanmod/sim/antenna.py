...
class ElemDipole(ElemBase):
    """
    Class for computing the dipole antenna element gain
    """
    def __init__(self, phi0=0, theta0=0, phibw=120, thetabw=65):
        """
        Constructor
    ...
    def response(self,phi,theta):
        ...
        # Put the theta from 0 to 180
        theta = 90-theta

        # antenna directivity
        theta = np.deg2rad(theta)
        eta = 377
        gain = (eta*(np.cos(0.5*np.pi*np.cos(theta))**2))/(2*np.pi*(np.sin(theta)**2)*36.5640)
